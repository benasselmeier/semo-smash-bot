// Simplified query for singles-only tournaments
const { getTournamentBySlug } = require('../utils/startgg');
const dataStore = require('../utils/dataStore');

module.exports = {
  name: 'import',
  description: 'Import singles matches from a tournament bracket into the database',
    ...
    try {
      ...
      for (let eventIndex = 0; eventIndex < singlesEvents.length; eventIndex++) {
        ...
        try {
          ...
          await loadingMessage.edit(`Importing data from event ${eventIndex + 1}/${singlesEvents.length}: ${event.name}...`);
          ...
          // Fetch bracket data for this event (simplified for singles only)
          await loadingMessage.edit(`Fetching data for event: ${event.name} (this may take a moment for large events)...`);
          const bracketData = await fetchSinglesEventBracket(event.id);
          ...
          for (let i = 0; i < bracketData.sets.length; i += batchSize) {
            ...
            for (const set of currentBatch) {
              ...
              const matchData = {
                ...
                setId: set.id,
                characters: {}, // Character data not available
...
/**
 * Fetch bracket data for a singles event from Start.gg API with reduced complexity
 * @param {string} eventId - Event ID
 * @returns {Promise<object>} Bracket data
 */
async function fetchSinglesEventBracket(eventId) {
  try {
    const { gql } = await import('graphql-request');
    const { queryStartGG } = require('../utils/startgg');
    
    // Use fallback strategy for high complexity
    let usingFallbackStrategy = false;
    let fallbackAttempted = false;
    
    // Function to fetch sets with fallback for high complexity
    async function fetchSetsWithFallback() {
      // Get all sets using pagination with minimal page size
      let allSets = [];
      let page = 1;
      const perPage = usingFallbackStrategy ? 3 : 5; // Extremely small page size to reduce complexity
      ...
      try {
        while (hasMorePages) {
          ...
          const query = usingFallbackStrategy ? 
            ...
            ` :
            // Very minimal query for singles events
            gql`
          ...
          if (result.event.sets && result.event.sets.nodes) {
            ...
            const maxSets = usingFallbackStrategy ? 50 : 100;
            ...
            if (hasMorePages) {
              const delay = usingFallbackStrategy ? 5000 : 3000;
    ...
    console.error('Error fetching singles event bracket:', error);
    ...
    if (error.response && error.response.errors) {
      ...
      if (graphqlErrors.includes('query complexity')) {
        throw new Error(`Start.gg API Query Complexity Error: Your query is too complex. The singles-only mode has a strict limit on set count (maximum 100 sets). Try reducing the page size further in the code if needed.`);
      }
